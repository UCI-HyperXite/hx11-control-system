...
{
	int16_t raw = (int16_t)Read16(ina219, INA219_REG_SHUNTVOLTAGE);
...
float INA219_GetTotalPowerUsed(INA219_t *ina219)
{
	...
	ina219->totalEnergy += (sample*1000);
...
float INA219_GetAVGMiliWatt(INA219_t *ina219)
{
	...
	{
		ina219->isFirst = true;
		ina219->sample[0] = INA219_ReadPower(ina219);
		ina219->index = 1;
	}
	else
	{
	...
	for(int i=0; i< (sizeof(ina219->sample)/sizeof(ina219->sample[0])); i++)
	{
		energy += ina219->sample[i];
	}
	energy = energy / (sizeof(ina219->sample)/sizeof(ina219->sample[0]));
...
void INA219_setCalibration_32V_2A(INA219_t *ina219)
{
	...
	ina219->powerMultiplier_mW = 2.0f; // Power LSB = 1mW per bit (2/1)
