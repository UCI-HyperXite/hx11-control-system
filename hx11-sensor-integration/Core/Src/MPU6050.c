...
{
	HAL_I2C_Mem_Write(MPU6050_I2C_Handler, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &val, 1, I2C_TIMEOUT);
}

void MPU6050_Writebytes(uint8_t reg_addr, uint8_t len, uint8_t* data)
{
	__disable_irq();
	HAL_I2C_Mem_Write(MPU6050_I2C_Handler, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, I2C_TIMEOUT);
	__enable_irq();
}

void MPU6050_Readbyte(uint8_t reg_addr, uint8_t* data)
{
	HAL_I2C_Mem_Read(MPU6050_I2C_Handler, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, 1, I2C_TIMEOUT);
}

void MPU6050_Readbytes(uint8_t reg_addr, uint8_t len, uint8_t* data)
{
	HAL_I2C_Mem_Read(MPU6050_I2C_Handler, MPU6050_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, I2C_TIMEOUT);
}

void MPU6050_Initialization(I2C_HandleTypeDef *handler)
{
	MPU6050_I2C_Handler = handler;
	HAL_Delay(50);
	uint8_t who_am_i = 0;
